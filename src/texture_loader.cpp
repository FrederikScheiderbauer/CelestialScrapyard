...
Texture TextureLoader::generate_One_Texture_As_cubemap(std::string texturepath)
{
    Texture texture;
    glGenTextures(1,&texture.id);
    glBindTexture(GL_TEXTURE_CUBE_MAP, texture.id);
    std::vector<std::string> cubemap_texturePaths;
    for (int i = 0; i < 6;i++) {
        cubemap_texturePaths.push_back(texturepath);
    }
    int width, height, nrChannels;
    for (unsigned int i = 0; i < cubemap_texturePaths.size(); i++)
    {
